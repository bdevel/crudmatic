module CrudmaticControllerMethods
  ...
  included do
    ...
    class_attribute :_crudmatic_model_class
    class_attribute :_crudmatic_actions
    class_attribute :_crudmatic_search_attributes
    
    # Set defaults
    self._crudmatic_actions = [:index, :show, :create, :update, :destroy, :bulk_edit]
  end

  class_methods do
    def crudmatic_controller_for(model_class)
      self._crudmatic_model_class = model_class
    end

    def crudmatic_actions(actions_array)
      self._crudmatic_actions = actions_array
    end

    def crudmatic_search_attributes(attrs_array)
      self._crudmatic_search_attributes = attrs_array
    end
  end
  ...
  def model_class
    from_var = self.class._crudmatic_model_class
    ...
    if from_var.nil?
      ...
      begin
        ...
        raise NameError.new("Cannot find model for controller #{self.class}. Assumed to be #{klass_name.inspect}. Set with crudmatic_controller_for ModelName in the controller.")
      end
    else
      from_var
    end
  ...
  def crud_actions
    self.class._crudmatic_actions || [:index, :show, :create, :update, :destroy]
  end
  
  def pagination_limit
    if model_class.respond_to?(:crudmatic_config)
      model_class.crudmatic_config.pagination_limit
    else
      50 # default fallback
    end
  end
  ...
  def index_order
    # Use crudmatic config or fallback
    if model_class.respond_to?(:crudmatic_config)
      model_class.crudmatic_config.index_order
    else
