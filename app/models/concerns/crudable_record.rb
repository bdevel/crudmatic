module CrudableRecord
  extend ActiveSupport::Concern

  included do
    # Instance method to access crudable config
    def crudable_config
      self.class.crudable_config
    end
  end
  ...
  class_methods do
    ...
    # Main crudable method to configure the model
    def crudable(type, *args)
      crudable_config.add_config(type, *args)
    end

    # Access to the config object
    def crudable_config
      @crudable_config ||= Crudable::RecordConfig.new(self)
    end

    # Backward compatibility methods that delegate to the config
    def show_attributes
      crudable_config.get_attributes(:show_attributes)
    end

    def index_attributes
      crudable_config.get_attributes(:index_attributes)
    end
    ...
    def permitted_attributes
      crudable_config.permitted_attributes
    end
  end
end